...
export const Categories = ({ addToFavorites, addToCart }) => {
  ...
  const categoryImages = {
    ...
    suplementos: "https://images.unsplash.com/photo-1579758629938-03607ccdbaba?q=80&w=600&auto=format&fit=crop",
    equipamiento: "https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=600&auto=format&fit=crop"
  };

  const uniqueCategories = [
    ...new Set(products.map((p) => p.category?.description).filter(Boolean))
  ];

  return (
    <div className="space-y-12 my-8">
      {uniqueCategories.map((category) => {
        const categoryProducts = products.filter((p) => p.category?.description === category);
        const bannerImage = categoryImages[category?.toLowerCase()] || "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=600&auto=format&fit=crop";
